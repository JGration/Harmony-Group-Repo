...
function thegem_setup() {
	...
	if(!get_option('pw_options')) {
		$pw_options = array(
			...
			'sidebars' => array('page-sidebar', 'footer-widget-area', 'shop-sidebar', 'page-sidebar-1', 'page-sidebar-2', 'shop-widget-area'),
		);
...
function thegem_admin_scripts_init() {
	...
	wp_enqueue_script('thegem-admin-functions', get_template_directory_uri() . '/js/thegem-admin_functions.js', array('jquery'), '1.2');
...
function thegem_title($sep = '&raquo;', $display = true, $seplocation = '') {
	...
	if(is_search()) {
		...
		$title = sprintf(wp_kses(__('<span class="light">Search Results</span> <span class="highlight gem-styled-color-1">"%1$s"</span>', 'thegem'), array('span' => array('class' => array()))), strip_tags($search));
	}
...
if(!function_exists('thegem_page_title')) {
	function thegem_page_title() {
		...
		if($title_style == 2 && !empty($page_data['title_template']) && get_post($page_data['title_template'])) {
...
function thegem_activate_elementor_settings() {
	...
	$cpt = array_merge($cpt, array('post', 'page', 'thegem_pf_item', 'thegem_title', 'thegem_footer', 'product'));
